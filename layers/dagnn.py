...
class DAGNN(nn.Module):
    ...
    def __init__(self, num_vocab, max_seq_len, emb_dim, hidden_dim, out_dim,
                 ...
                 agg_x=False, agg="attn_h", out_wx=True, out_pool_all=True, out_pool="mean", encoder=None, dropout=0.0,
        ...
        if "self_attn" in agg:
            ...
            #Self Attention (list 0 for forward direction, list 1 for reverse direction)
            self.node_aggr_0 = nn.ModuleList([
                #attn_dim,num_relations
            ...
            # Multi Attention / Cross Attention? (list 0 for forward direction, list 1 for reverse direction)
            ...
            #Gated sum convolution (list 0 for forward direction, list 1 for reverse direction)
            self.node_aggr_0 = nn.ModuleList([
    ...
    def _out_nodes_self_attn(self, h, batch):
        attn_weights = self.self_attn_linear_out(h) #self.self_attn_linear_out = torch.nn.Linear(d, 1)
